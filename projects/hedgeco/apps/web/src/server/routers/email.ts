...
export const emailRouter = router({
  ...
  /**
   * Send investor/manager registration autoresponder
   */
  sendInvestorManagerRegistration: adminProcedure
    .input(
      z.object({
        userId: z.string(),
        type: z.enum(['investor', 'manager']),
      })
    )
    .mutation(async ({ input }) => {
      const user = await prisma.user.findUnique({
      ...
      const confirmUrl = `${process.env.NEXT_PUBLIC_APP_URL}/api/auth/confirm-registration?token=${user.emailVerificationToken}&userId=${user.id}`;
      const firstName = user.profile?.firstName || user.email.split('@')[0];

      const result = await sendInvestorManagerRegistrationEmail(
   ...
   */
  sendServiceProviderRegistration: adminProcedure
    .input(z.object({ userId: z.string() }))
    .mutation(async ({ input }) => {
      const user = await prisma.user.findUnique({
        where: { id: input.userId },
      ...
      const result = await sendServiceProviderRegistrationEmail(
        { email: user.email, firstName },
        confirmUrl
      );
   ...
   */
  sendNewsMemberRegistration: adminProcedure
    .input(z.object({ userId: z.string() }))
    .mutation(async ({ input }) => {
      const user = await prisma.user.findUnique({
        where: { id: input.userId },
      ...
      const activateUrl = `${process.env.NEXT_PUBLIC_APP_URL}/api/auth/activate-news?token=${user.emailVerificationToken}&userId=${user.id}`;
      const firstName = user.profile?.firstName || user.email.split('@')[0];

      const result = await sendNewsMemberRegistrationEmail(
        { email: user.email, firstName },
        activateUrl
      );
   ...
   */
  sendAdminNewRegistrationAlert: adminProcedure
    .input(z.object({ userId: z.string() }))
    .mutation(async ({ input }) => {
      const user = await prisma.user.findUnique({
        where: { id: input.userId },
      ...
      const result = await sendAdminNewRegistrationNotification(
        adminEmail,
        {
          id: user.id,
          email: user.email,
